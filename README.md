...
Kindly read this article for the statistical method and modeling assumptions used by each tool: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7330487/.
...
##  Comparison of Methods for Heritability

This table presents a comparison of different methods used to calculate heritability in this research work. The first column shows the index of each method, the second column shows the tools used to calculate the heritability, and the third column shows the data and algorithm used to create a unique variant of each method. The GWAS, genotype data, covariates, and reference panel columns indicate whether specific data was used for the analysis or not. The last column indicates whether clumping or pruning was performed on the genotype data before it was used in the analysis. The final column shows the number of unique heritability values generated by each method.  
...
Execute the command below. It will check the build, and generate the files for a specific phenotype that includes covariate and bed bim and fam file. The code works for binary phenotype as well.
